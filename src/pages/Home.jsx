      ...
      async (p) => {
        try {
          ...
          const res = await axios.get(
            `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current_weather=true&hourly=precipitation`,
            {
              withCredentials: false, // 🔥 force disable
              timeout: 5000,
            },
          );
          const currentHour = new Date().getHours();
          ...
          setWeather({
            temperature: res?.data?.current_weather?.temperature ?? 0,
            windspeed: res?.data?.current_weather?.windspeed ?? 0,
            rainfall: res?.data?.hourly?.precipitation?.[currentHour] ?? 0,
          });
        } catch (err) {
